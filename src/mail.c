...
staticfn NEARDATA const char *mail_text[] = { "让一让!", "当心!",
                                            "打扰一下!" };
...
readmail(struct obj *otmp UNUSED)
{
    ...
    static const char *const junk_templates[] = {
        "%s请向<%s>报告错误.%s", /*** must be first entry ***/
        "请忽略前一封邮件.",
        "欢迎来到NetHack.",
#ifdef AMIGA
        "只有Amiga办得到.",
        "**猫**知道所有答案.",
#endif
        "本邮件符合岩德利亚反垃圾邮件法案(Yendorian Anti-Spam Act)的规定",
        "随信附上一份小礼物, 以此代表您的枭熊",
    ...
    if (strchr(junk_templates[i], '%')) {
        ...
        } else if (strstri(junk_templates[i], "网站")) { //危险???
...
read_simplemail(const char *mbox, boolean adminmsg)
{
    ...
    while (fgets(curline, 128, mb) != NULL) {
...
#endif
            pline("这张卷轴上有%s一条信息.", /*换pline:There*/
                  seen_one_already ? "另" : "");
        }
        msg = strchr(curline, ':');
        ...
        *msg = '\0';
        ...
        if (adminmsg) {
            urgent_pline("%s的声音响彻洞穴:",
                         curline);
        } else {
            pline("这条信息是'%s'发来的.", curline);
            pline("上面写着:");
        }
